...
dexSpy.Parent = game:GetService("CoreGui")
...
do
	...
	local keywords = {
		...
		{
			["Keyword"] = "InvokeServer",
			["Color"] = Color3.fromRGB(123, 216, 143)
		},
		{
			["Keyword"] = "game",
			["Color"] = Color3.fromRGB(252, 97, 141)
		},
		{
			["Keyword"] = "for",
			["Color"] = Color3.fromRGB(252, 97, 141)
		...
		{
			["Keyword"] = "true",
			["Color"] = Color3.fromRGB(122, 114, 185)
		},
		{
			["Keyword"] = "false",
			["Color"] = Color3.fromRGB(122, 114, 185)
			...
			for i, v in pairs(keywords) do
				if not inString and not inComment and char == v["Keyword"]:sub(1, 1) and text:sub(pos + 1, pos + #v["Keyword"]) == v["Keyword"] then
					if not v["Independent"] and not text:sub(pos, pos):match("%w") and not text:sub((pos + #v["Keyword"]) + 1, (pos + #v["Keyword"]) + 1):match("%w") then
						...
						testLabel.Parent = game:GetService("CoreGui")
	...
	local buttons = {
		["copyBtn"] = function()
			if currentButton then
				setclipboard(logs[currentButton]["Loading Script"])
			end
		end,
		["clearBtn"] = function()
			logs = {}
		...
		["exitBtn"] = function()
			currentRemote = nil
			mainFrame:TweenSize(UDim2.new(0, 565, 0, 28), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.25)
			wait(0.25)
			mainFrame:TweenSize(UDim2.new(0, 0, 0, 28), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.25)
			wait(0.25)
			dexSpy.Enabled = false
		...
		["copyRemBtn"] = function()
			if currentButton then
				setclipboard(transformPath(logs[currentButton]["Remote"]:GetFullName()))
			end
		end,
		["copyScrBtn"] = function()
			if currentButton then
				setclipboard(transformPath(logs[currentButton]["Calling Script"]:GetFullName()))
			end
		end
	}
...
local function log(remote,...)
	...
	remoteButton.MouseEnter:Connect(function()
		...
		if remoteButton ~= currentButton then
			tweenService:Create(remoteButton, TweenInfo.new(0.1), {
	...
	remoteButton.MouseButton1Click:Connect(function()
		setthreadcontext(6)
		if currentButton then
			currentButton.BackgroundTransparency = 1
		end
...
scriptDumper.Parent = game:GetService("CoreGui")
do
	draggable(mainFrame)
	local buttons = {
		["copyBtn"] = function()
			if currentButton then
				setclipboard(dlogs[currentButton]["Loading Dump"])
			end
		end,
		["clearBtn"] = function()
			dlogs = {}
			for i, v in pairs(scriptsScrollingFrame:GetChildren()) do
				if not v:IsA("UIListLayout") then
					v:Destroy()
				end
			end
...
local function log(script)
	...
	scriptButton.MouseEnter:Connect(function()
		if scriptButton ~= currentButton then
			tweenService:Create(scriptButton, TweenInfo.new(0.1), {
				BackgroundTransparency = 0
			}):Play()
		end
	end)
...
D_E_X.Parent = game:GetService("CoreGui").RobloxGui
spawn(function()
	local Gui = D_E_X
	...
	SaveMapButton.MouseButton1Click:Connect(function()
			...
			for i, v in pairs(root:GetChildren()) do
				pcall(function()
					if v:IsA("LocalScript") then
						...
						if v.Source == "" then
							if consoleFunc then
								consoleFunc("LocalScript " .. v.Name .. " had a problem decompiling.")
							end
						else
							if consoleFunc then
								consoleFunc("LocalScript " .. v.Name .. " decompiled.")
							end
						end
					elseif v:IsA("ModuleScript") then
						...
						if v.Source == "" then
							if consoleFunc then
								consoleFunc("ModuleScript " .. v.Name .. " had a problem decompiling.")
							end
						else
							if consoleFunc then
								consoleFunc("ModuleScript " .. v.Name .. " decompiled.")
							end
						end
					end
				end)
		...
		function saveProp(obj, prop, par)
			...
			elseif type(myProp) == "string" then
				...
				newProp.Name = prop
				newProp.Value = myProp
			elseif type(myProp) == "userdata" then
				pcall(function()
...
spawn(function()
	...
	do
		...
		local function ArrowGraphic(size, dir, scaled, template)
			...
			if scaled then
				function scale(p, s)
					return UDim2.new(p.X.Offset / size, 0, p.Y.Offset / size, 0), UDim2.new(s.X.Offset / size, 0, s.Y.Offset / size, 0)
				end
			else
				function scale(p, s)
					return p, s
	...
	function CreateRightClickMenuItem(text, onClick, insObj)
		...
		button.MouseEnter:Connect(function()
			TweenService:Create(button, TweenInfo.new(0.1), {
				TextColor3 = DropDown.TextColorOver,
				BackgroundColor3 = DropDown.BackColorOver
			}):Play()
	...
	function rightClickMenu(sObj)
		...
		currentRightClickMenu = CreateRightClickMenu(options, "", false, function(option)
			if option == "Cut" then
				if not Option.Modifiable then
					return 
				end
						...
						expand.MouseEnter:Connect(function()
							...
							if #node > 0 then
								if node.Expanded then
									...
									TweenService:Create(earrow.IconMap, TweenInfo.new(0.1), {
										ImageColor3 = Color3.fromRGB(197, 28, 70)
									}):Play()
								else
...
spawn(function()
	...
	for i, v in pairs(getrenv()) do
		if type(v) == 'function' and type(i) == 'string' then
			table.insert(syntaxHighlightList, {
				['Keyword'] = i,
				["Color"] = Color3.fromRGB(123, 216, 143),
				["Independent"] = true
			})
	...
	function buildScript(source, xOff, yOff, override)
		...
		while true do
			...
			while entry ~= nil and typeof(entry) == 'string' and string.len(entry) > 0 do
				...
				local oldTC3 = Color3.fromRGB(244, 244, 244)
				if colorTime > 0 then
					...
					grid[currentColumn][currentRow].TextColor3 = oldTC3
					inString = false
				end
				if delayance == 0 then
